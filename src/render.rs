...
impl Camera {
    pub fn new(position: Vector3, direction: Vector3, width: u32, height: u32, fov: f64) -> Self {
        Self { position, direction, width, height, fov, rays: Vec::new() }
    }
    ...
    pub fn initialize_rays(&mut self) {
        ...
        for y in 0..self.height {
            ...
            for x in 0..self.width {
                ...
                row.push(Ray::new(self.position.clone(), Vector3::new(
        ...
        for y in 0..self.camera.height {
            for x in 0..self.camera.width {
                if self.camera.get_ray(x, y) {
                    ...
                    let color = ray.color.clone() * ray.brightness;
                    image.put_pixel(x as u32, y as u32, Rgb([color.x as u8, color.y as u8, color.z as u8]));
                } else {
